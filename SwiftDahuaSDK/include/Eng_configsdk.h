...
#define MAX_TEMPLATEREGION_NUM  32                  // 
#define POINT_PAIR_NUM          2                   //
...
#define MAX_TRIGGER_MODE_NUMBER             64
...
#define CFG_CMD_RAINBRUSHMODE           "RainBrushMode"         // Rain brush mode(对应CFG_RAINBRUSHMODE_INFO数组)
...
typedef enum tagCFG_H264_PROFILE_RANK
{
	...
	PROFILE_HIGH,                                   // FRExt￡?Main_Profile Based on the new￡o8x8 intra prediction(8x8 Intra-frame Predicdion), custom 
...
// multi preview work mode
typedef enum tagCFG_EM_PREVIEW_MODE
{
    CFG_EM_PREVIEW_MODE_UNKNOWN = 0,        // 
    CFG_EM_PREVIEW_MODE_SNAPSHOT,           // snapshot mode
    CFG_EM_PREVIEW_MODE_SPLITENCODE,        // splitencode mode
    CFG_EM_PREVIEW_MODE_SPLITSNAP,          // splitsnap mode
}CFG_EM_PREVIEW_MODE;

#define MAX_PREVIEW_MODE_SPLIT_TYPE_NUM     8                           // max kinds of multi preview split

// capacity of encode
typedef struct tagCFG_ENCODECAP
{
	int                     nChannelNum;                                // channel number
	CFG_VIDEO_ENCODECAP     stuVideoEncodeCap[MAX_VIDEO_CHANNEL_NUM];	// ability information array of each channel
    CFG_EM_PREVIEW_MODE     emPreviewMode;                              // work mode of multi preview
    int                     nSplitModeNum;                              // number of splite mode
    int                     anSplitMode[MAX_PREVIEW_MODE_SPLIT_TYPE_NUM];// splite information
                                                                        // 
}CFG_ENCODECAP;
...
typedef enum tagEM_CFG_DEFENCEAREATYPE
{
    ...
    EM_CFG_DefenceAreaType_Fire,         
    EM_CFG_DefenceAreaType_FullDaySound, 
    EM_CFG_DefenceAreaType_FullDaySlient, 
    EM_CFG_DefenceAreaType_Entrance1,    
    EM_CFG_DefenceAreaType_Entrance2,  
    EM_CFG_DefenceAreaType_InSide,  
...
typedef struct tagCFG_ALARMIN_INFO
{
    ...
    int                 nLevel2;                                    // 2rd level cascading address, means connected to no. nLevel1 meter?ˉs detector no., -1 means invalid, from 0
} CFG_ALARMIN_INFO;
...
typedef struct tagCFG_MOTION_INFO 
{
	...
	BOOL                 abDetectRegion;                            // Read only￡?1￡onRegionCount￡?stuRegion valid
...
typedef struct tagCFG_TRAFFIC_SCENE_INFO 
{
	...
	float				fCameraHeight;							// The Camera Height from the Ground	Unit￡ometer
	float				fCameraDistance;						// Camera Horizontal distance from the Center of a Ground Test Area	Unit￡ometer
...
{
	float				fCameraHeight;							// Camera distance to ground	unit￡om
	float				fCameraAngle;							// Camera and vertical direction angle	unit degree￡?0~90￡?
	float				fCameraDistance;						// Camera distance to ground center?ˉs horizontal distance	unit￡om
...
{
	int                    nPositionX;           //PTZ Horizontal Motion Position, the effective range ￡o[0,3600]
...
typedef struct tagCFG_VIDEOABNORMALDETECTION_INFO
{
	...
	BYTE                bDetectType[MAX_ABNORMAL_DETECT_TYPE];                  // Detect type,0-video lost, 1-video blind, 2-video freeze, 3- too light, 4-too dark, 5-sence change
                                                                                // 6-stripe detect , 7-noise detect , 8-偏色检测 , 9-视频模糊检测 , 10-constrast abnormal
                                                                                // 11-视频运动 , 12-video flash , 13-video color , 14-虚焦检测 , 15-over expose
	int					nMinDuration;											// Min duration s 0~65535
	CFG_ALARM_MSG_HANDLE stuEventHandler;										// Alarm event handler
	CFG_TIME_SECTION	stuTimeSection[WEEK_DAY_NUM][MAX_REC_TSECT_EX];			// Event respond time section
...
typedef struct tagCFG_TRAFFIC_VEHICLEINBUSROUTE_INFO
{
    ...
    BYTE                byCarType;                                              // Snapshot vehicle type￡?0￡onon-yellow plate 1￡oblue plate 2￡onon-truck
    BYTE                bSensitivity;                                           // Sensitivity￡?value 1-10￡?the higher the value the higher the sensitivity
    BYTE                bReserved[2];                                           // Reserved text
    int					nObjectTypeNum;											// Corresponding object type number
    char				szObjectTypes[MAX_OBJECT_LIST_SIZE][MAX_NAME_LEN];		// Corresponding object type list
...
typedef struct tagCFG_IVS_GETOUTBEDDETECTION_INFO
{
    ...
    BYTE                bReserved[3];                                       
    int					nObjectTypeNum;										
    char				szObjectTypes[MAX_OBJECT_LIST_SIZE][MAX_NAME_LEN];	
    int                 nPtzPresetId;										
    CFG_ALARM_MSG_HANDLE stuEventHandler;									
    CFG_TIME_SECTION	stuTimeSection[WEEK_DAY_NUM][MAX_REC_TSECT_EX];			

    int					nDetectRegionPoint;									
    CFG_POLYGON			stuDetectRegion[MAX_POLYGON_NUM];					
    unsigned int        nMinDuration;                                           
    unsigned int        nReportInterval;                                      
    int                 nAssisDectLinePointNum;                               
...
typedef struct tagCFG_IVS_ONDUTYDETECTION_INFO
{
    ...
    unsigned int        nMinDuration;                                          
    unsigned int        nReportInterval;                                       
    int                 nTemplateRegionPointPairNum;                         
    CFG_POLYGON         stuTemplateRegion[MAX_TEMPLATEREGION_NUM][POINT_PAIR_NUM];  
...
{
	int nHeight;			//Black border height, range￡o0 ~ ( 8192-original height)
...
typedef struct tagCFG_VT_NOANSWER_FORWARD_INFO
{
    ...
    int                             nGroupNum;                                      // No-response transfer config list valid number￡?range￡o0 ~ CFG_MAX_NOANSWER_FORWARD_GROUP_NUM
...
typedef struct tagCFG_PTZ_LIGHTING_CONTROL
{
	char                szMode[CFG_COMMON_STRING_32];  // Manual Lighting Control Mode 
	                                                   ...
	                                                   // "adjustLight"￡oManually Adjust Brightness Mode 
...
typedef struct tagCFG_AUDIO_INPUT 
{
    char        szAudioSource[CFG_COMMON_STRING_256];   // Input audio source, if audio channel input is composed of multiple channels, use | to split.
                                                        // i.e.￡o"Mic|LineIn|Remote" means this audio channel is composed of Mic, LineIn and remote channel audio input.
                                                        // "Coaxial" 
                                                        // "BNC" 
...
{
    BOOL			bEnable;				// Enable
    int             nInterval;              // Mail sending interval, unit is s, range￡o0~3600
...
typedef struct tagCFG_AIRCONDITION_DETAIL
{
    ...
    int                 nRange;                                 // Range value(temperature)unit￡oCelsius
...
typedef struct tagCFG_COMPRESS_PLAY_INFO
{
    BOOL                bEnable;                // Enable
	CFG_CAPTURE_SIZE    emResolution;           // Video resolution
	unsigned int        nBitRate;               // Video fixed stream value(kbps), range￡o192~1024
...
{
    int                 nTankVolume;            // ank capacity￡?unit￡oL
    int                 nNotifyIntervalTime;    // report tank info interval, unit ￡os
    int                 nLowOil;                // tank oil level low alarm percentage(i.e.￡obelow 20% to alarm), unit￡opercentage￡?0￡-100
    int                 nAlarmIntervalTime;     // report oil level low alarm time interval(as after alarms, alarm is reapeat periodically), unit￡os
}CFG_OIL_MASS_INFO; 
...
typedef struct tagCFG_STANDING_TIME_INFO
{
    ...
    int                         nUpperlimit;     // duration limit, unit￡omin
    int                         nInterval;       // DH_ALARM_VEHICLE_STANDING_OVER_TIME event report interval, unit￡os
...
typedef struct tagCFG_LANES_STATE_REPORT
{
    ...
    int                 nPeriod;        //Report period	unit￡os
...
typedef enum tagEM_CFG_BURNPLAN_MODE
{
    ...
    EM_CFG_BURNPLAN_MODE_SYNC,              // Sync￡o "Sync"
...
{
    char                            szDevType[CFG_COMMON_STRING_128];       // 
    char                            szDevName[CFG_COMMON_STRING_128];       // 
    char                            szIP[MAX_ADDRESS_LEN];                  // 
    int                             nPort;                                  //
    char                            szUser[CFG_COMMON_STRING_64];           // 
    char                            szPassword[CFG_COMMON_STRING_64];       // 
    char                            szDevID[CFG_COMMON_STRING_128];         // 
    char                            szParentID[CFG_COMMON_STRING_128];      // 
}CFG_DEV_LIST;


typedef struct tagCFG_DEV_LIST_INFO 
{
    CFG_DEV_LIST*                   pstuDevList;                            // 
    DWORD                           dwDevListMaxCount;                      // 
    DWORD                           dwDevListRetCount;                      // 
}CFG_DEV_LIST_INFO;

#define MAX_CALIBRATE_MATRIX_LEN    8       
...
{
    EM_CFG_LIGHTING_MODE_UNKNOWN,               // Uknow
    EM_CFG_LIGHTING_MODE_MANUAL,                // Manual operation: "Manual"
    EM_CFG_LIGHTING_MODE_ZOOMPRIO,              //Ratio is preferred: "ZoomPrio"
